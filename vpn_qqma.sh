...
USERNAME=$(whoami)
HOSTNAME=$(hostname)
export UUID=${UUID:-'8a4ca30b-190b-4348-8871-a45559757aea'}
...
run_sb() {
  ...
  if [ -e bot ]; then
    nohup ./bot tunnel --edge-ip-version auto --no-autoupdate --protocol http2 --logfile boot.log --loglevel info --url http://localhost:$vmess_port >/dev/null 2>&1 &
    sleep 2
    pgrep -x "bot" >/dev/null && green "bot is running" || {
      red "bot is not running, restarting..."
      pkill -x "bot" && nohup ./bot "${tunnel --edge-ip-version auto --no-autoupdate --protocol http2 --logfile boot.log --loglevel info --url http://localhost:$vmess_port}" >/dev/null 2>&1 &
      sleep 2
